...
const PatientDetail = () => {
	...
	const age = calculateAge(patient.dobAD);
				...
				<div className="flex items-center gap-4">
					...
					<div>
						<div className="flex items-center gap-3 flex-wrap">
							...
							{patient.fullNameNe && (
								<span className="text-gray-400 font-nepali text-lg">{patient.fullNameNe}</span>
							)}
						</div>
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
				<div className="md:col-span-2 bg-white rounded-xl border border-gray-200 p-5">
					...
					<dl className="space-y-1">
						<DetailRow label="Date of Birth" value={patient.dobAD ? formatDate(patient.dobAD) : null} />
					...
					{patient.insuranceProvider && (
						<div className="mt-4 pt-4 border-t">
							...
							{patient.insurancePolicyNo && (
								<p className="text-xs text-gray-400">{patient.insurancePolicyNo}</p>
							)}
						</div>
					)}
				</div>
							...
							<div className="mb-3">
								...
								<div className="flex flex-wrap gap-1">
									{patient.allergies.map((a) => (
										<span key={a} className="text-xs bg-red-100 text-red-700 px-2 py-0.5 rounded-full">
											{a}
										</span>
									))}
								</div>
							...
							<div>
								...
								<div className="flex flex-wrap gap-1">
									{patient.chronicConditions.map((c) => (
										<span key={c} className="text-xs bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full">
											{c}
										</span>
									))}
								</div>
